...
auto ComputeNoiseEstimate(
    const std::vector<Eigen::Vector3d>& points,
    const std::vector<Eigen::Vector3d>& normals,
    const Eigen::VectorXd& weights,
    const std::vector<Eigen::Matrix3d>& normal_covariances,
    ...
    const T& stdevPoints)
{
    ...
    for (size_t i = 0; i < nPoints; i++) {
        ...
        const T w = static_cast<T>(weights[i]);  // 从Eigen::VectorXd转换权重
        ...
        N.block(3, 3, 3, 3) = normal_covariances[i].template cast<T>();
...
template <typename T>
Eigen::VectorXd SolveWithSnrProbabilities(
    const Eigen::MatrixXd& U,                  // 特征向量矩阵
    const Eigen::VectorXd& eigenvalues,        // 特征值
    const Eigen::VectorXd& rhs,                // 右侧向量
    const Eigen::VectorXd& snr_probabilities)  // SNR概率
{
    const size_t dim = eigenvalues.size();
    Eigen::VectorXd d_psinv = Eigen::VectorXd::Zero(dim);
    ...
    for (size_t i = 0; i < dim; i++) {
        ...
        // 避免除以非常小的特征值
        if (std::abs(eigenvalue) > 1e-10) {
            d_psinv[i] = p / eigenvalue;
        }
    }

    // 计算扰动向量
    Eigen::VectorXd perturbation = U * d_psinv.asDiagonal() * U.transpose() * rhs;
